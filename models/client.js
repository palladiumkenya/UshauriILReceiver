...
function validateClient(client) {
	const schema = {
		group_id: Joi.number(),
		mfl_code: Joi.number().min(5).max(5),
		clinic_number: Joi.number().min(10).max(10),
		file_no: Joi.string(),
		...
		m_name: Joi.string().min(3).max(10),
		l_name: Joi.string().min(3).max(10).required(),
		dob: Joi.date().required(),
		phone_no: Joi.string().max(10).min(10),
		status: Joi.string(),
		clinic_id: Joi.number(),
		prep_number: Joi.number().min(14).max(14)
	};
